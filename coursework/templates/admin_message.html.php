<h2>Message #<?= (int)$message['message_id'] ?></h2>

<?php
if (!empty($message['user_id'])) {
    $sender = htmlspecialchars($message['username'] ?? '', ENT_QUOTES, 'UTF-8');
    $senderEmail = htmlspecialchars($message['email'] ?? '', ENT_QUOTES, 'UTF-8');
} else {
    $sender = htmlspecialchars($message['sender_name'] ?? '', ENT_QUOTES, 'UTF-8');
    $senderEmail = htmlspecialchars($message['sender_email'] ?? '', ENT_QUOTES, 'UTF-8');
}
?>

<p><strong>From:</strong> <?= $sender ?><?php if ($senderEmail): ?> (<?= $senderEmail ?>)<?php endif; ?></p>
<p><strong>When:</strong> <?=htmlspecialchars($message['created_at'] ?? '', ENT_QUOTES, 'UTF-8')?></p>
<p><strong>Subject:</strong> <?=htmlspecialchars($message['subject'] ?? '', ENT_QUOTES, 'UTF-8')?></p>
<hr>
<div style="white-space:pre-wrap;"><?=htmlspecialchars($message['content'] ?? '', ENT_QUOTES, 'UTF-8')?></div>
<hr>
<form action="admin_message.php?id=<?= (int)$message['message_id'] ?>" method="post">
    <label for="status">Status</label>
    <select id="status" name="status">
        <option value="new" <?=($message['status']==='new' ? 'selected' : '')?>>new</option>
        <option value="resolved" <?=($message['status']==='resolved' ? 'selected' : '')?>>resolved</option>
    </select>
    <input type="submit" value="Update">
</form>
